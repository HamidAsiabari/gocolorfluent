...
export default function Home() {
  ...
  // Key Components Refs
  const movinPlateRef = useRef<THREE.Object3D | null>(null)
  const upperSideMainHolderRef = useRef<THREE.Object3D | null>(null)
  const lowerSideMainRef = useRef<THREE.Object3D | null>(null)
  const handleUpCoverRef = useRef<THREE.Object3D | null>(null)
  const hairGuideSupportRef = useRef<THREE.Object3D | null>(null)
  ...
  useEffect(() => {
    ...
    loader.load('/product-3d/Color_Brush_assembly_V1_1.glb', (gltf) => {
      ...
      // Find and store references to key components
      model.traverse((child) => {
        if (child.name === 'Movin_Plate1') {
          movinPlateRef.current = child
          console.log('Found Movin_Plate1:', child)
        } else if (child.name === 'Upper_side_main_holder1') {
          upperSideMainHolderRef.current = child
          console.log('Found Upper_side_main_holder1:', child)
        } else if (child.name === 'Lower_Side_Main1') {
          lowerSideMainRef.current = child
          console.log('Found Lower_Side_Main1:', child)
        } else if (child.name === 'Handle_up_cover1') {
          handleUpCoverRef.current = child
          console.log('Found Handle_up_cover1:', child)
        } else if (child.name === 'Hair_guide_Support1') {
          hairGuideSupportRef.current = child
          console.log('Found Hair_guide_Support1:', child)
        }
      })
  ...
  // Apply key components controls to 3D objects
  useEffect(() => {
    if (movinPlateRef.current) {
      movinPlateRef.current.position.set(
        keyComponents.movinPlate.position.x,
        keyComponents.movinPlate.position.y,
        keyComponents.movinPlate.position.z
      )
      movinPlateRef.current.rotation.set(
        keyComponents.movinPlate.rotation.x,
        keyComponents.movinPlate.rotation.y,
        keyComponents.movinPlate.rotation.z
      )
      movinPlateRef.current.visible = keyComponents.movinPlate.visible
    }
  }, [keyComponents.movinPlate])

  useEffect(() => {
    if (upperSideMainHolderRef.current) {
      upperSideMainHolderRef.current.position.set(
        keyComponents.upperSideMainHolder.position.x,
        keyComponents.upperSideMainHolder.position.y,
        keyComponents.upperSideMainHolder.position.z
      )
      upperSideMainHolderRef.current.rotation.set(
        keyComponents.upperSideMainHolder.rotation.x,
        keyComponents.upperSideMainHolder.rotation.y,
        keyComponents.upperSideMainHolder.rotation.z
      )
      upperSideMainHolderRef.current.visible = keyComponents.upperSideMainHolder.visible
    }
  }, [keyComponents.upperSideMainHolder])

  useEffect(() => {
    if (lowerSideMainRef.current) {
      lowerSideMainRef.current.position.set(
        keyComponents.lowerSideMain.position.x,
        keyComponents.lowerSideMain.position.y,
        keyComponents.lowerSideMain.position.z
      )
      lowerSideMainRef.current.rotation.set(
        keyComponents.lowerSideMain.rotation.x,
        keyComponents.lowerSideMain.rotation.y,
        keyComponents.lowerSideMain.rotation.z
      )
      lowerSideMainRef.current.visible = keyComponents.lowerSideMain.visible
    }
  }, [keyComponents.lowerSideMain])

  useEffect(() => {
    if (handleUpCoverRef.current) {
      handleUpCoverRef.current.position.set(
        keyComponents.handleUpCover.position.x,
        keyComponents.handleUpCover.position.y,
        keyComponents.handleUpCover.position.z
      )
      handleUpCoverRef.current.rotation.set(
        keyComponents.handleUpCover.rotation.x,
        keyComponents.handleUpCover.rotation.y,
        keyComponents.handleUpCover.rotation.z
      )
      handleUpCoverRef.current.visible = keyComponents.handleUpCover.visible
    }
  }, [keyComponents.handleUpCover])

  useEffect(() => {
    if (hairGuideSupportRef.current) {
      hairGuideSupportRef.current.position.set(
        keyComponents.hairGuideSupport.position.x,
        keyComponents.hairGuideSupport.position.y,
        keyComponents.hairGuideSupport.position.z
      )
      hairGuideSupportRef.current.rotation.set(
        keyComponents.hairGuideSupport.rotation.x,
        keyComponents.hairGuideSupport.rotation.y,
        keyComponents.hairGuideSupport.rotation.z
      )
      hairGuideSupportRef.current.visible = keyComponents.hairGuideSupport.visible
    }
  }, [keyComponents.hairGuideSupport])
      ...
      {isDevMode && (
        <div className="fixed bottom-4 left-4 z-20 bg-black/90 backdrop-blur-sm border border-gray-600 rounded-lg p-2 text-white max-w-80 max-h-[70vh] overflow-y-auto">
          ...
          <div className="space-y-2">
            ...
            <div className="bg-gray-800/50 p-2 rounded">
              <h4 className="text-xs font-medium text-purple-400 mb-2">🔧 Key Components Details</h4>
              
              {/* Movin Plate */}
              <div className="mb-3">
                <div className="flex items-center justify-between mb-1">
                  <h5 className="text-xs font-medium text-cyan-400">Movin Plate</h5>
                  <label className="flex items-center text-xs">
                    <input
                      type="checkbox"
                      checked={keyComponents.movinPlate.visible}
                      onChange={(e) => setKeyComponents(prev => ({
                        ...prev,
                        movinPlate: { ...prev.movinPlate, visible: e.target.checked }
                      }))}
                      className="mr-1"
                    />
                    Visible
                      ...
                      {['x', 'y', 'z'].map((axis) => (
                        <div key={axis} className="flex flex-col">
                          ...
                          <input
                            type="number"
                            step="0.1"
                            value={keyComponents.movinPlate.position[axis as keyof typeof keyComponents.movinPlate.position]}
                            onChange={(e) => setKeyComponents(prev => ({
                              ...prev,
                              movinPlate: {
                                ...prev.movinPlate,
                                position: { ...prev.movinPlate.position, [axis]: parseFloat(e.target.value) || 0 }
                              }
                            }))}
                            className="w-full px-1 py-0.5 text-xs bg-gray-700 border border-gray-600 rounded"
                          />
                        </div>
                      ))}
                    </div>
                      ...
                      {['x', 'y', 'z'].map((axis) => (
                        <div key={axis} className="flex flex-col">
                          ...
                          <input
                            type="number"
                            step="0.1"
                            value={keyComponents.movinPlate.rotation[axis as keyof typeof keyComponents.movinPlate.rotation]}
                            onChange={(e) => setKeyComponents(prev => ({
                              ...prev,
                              movinPlate: {
                                ...prev.movinPlate,
                                rotation: { ...prev.movinPlate.rotation, [axis]: parseFloat(e.target.value) || 0 }
                              }
                            }))}
                            className="w-full px-1 py-0.5 text-xs bg-gray-700 border border-gray-600 rounded"
                          />
              ...
              {/* Upper Side Main Holder */}
              <div className="mb-3">
                <div className="flex items-center justify-between mb-1">
                  <h5 className="text-xs font-medium text-cyan-400">Upper Side Main Holder</h5>
                  <label className="flex items-center text-xs">
                    <input
                      type="checkbox"
                      checked={keyComponents.upperSideMainHolder.visible}
                      onChange={(e) => setKeyComponents(prev => ({
                        ...prev,
                        upperSideMainHolder: { ...prev.upperSideMainHolder, visible: e.target.checked }
                      }))}
                      className="mr-1"
                    />
                    Visible
                      ...
                      {['x', 'y', 'z'].map((axis) => (
                        <div key={axis} className="flex flex-col">
                          ...
                          <input
                            type="number"
                            step="0.1"
                            value={keyComponents.upperSideMainHolder.position[axis as keyof typeof keyComponents.upperSideMainHolder.position]}
                            onChange={(e) => setKeyComponents(prev => ({
                              ...prev,
                              upperSideMainHolder: {
                                ...prev.upperSideMainHolder,
                                position: { ...prev.upperSideMainHolder.position, [axis]: parseFloat(e.target.value) || 0 }
                              }
                            }))}
                            className="w-full px-1 py-0.5 text-xs bg-gray-700 border border-gray-600 rounded"
                          />
                      ...
                      {['x', 'y', 'z'].map((axis) => (
                        <div key={axis} className="flex flex-col">
                          ...
                          <input
                            type="number"
                            step="0.1"
                            value={keyComponents.upperSideMainHolder.rotation[axis as keyof typeof keyComponents.upperSideMainHolder.rotation]}
                            onChange={(e) => setKeyComponents(prev => ({
                              ...prev,
                              upperSideMainHolder: {
                                ...prev.upperSideMainHolder,
                                rotation: { ...prev.upperSideMainHolder.rotation, [axis]: parseFloat(e.target.value) || 0 }
                              }
                            }))}
                            className="w-full px-1 py-0.5 text-xs bg-gray-700 border border-gray-600 rounded"
                          />
              ...
              <div className="mb-3">
                <div className="flex items-center justify-between mb-1">
                  <h5 className="text-xs font-medium text-cyan-400">Lower Side Main</h5>
                  <label className="flex items-center text-xs">
                    <input
                      type="checkbox"
                      checked={keyComponents.lowerSideMain.visible}
                      onChange={(e) => setKeyComponents(prev => ({
                        ...prev,
                        lowerSideMain: { ...prev.lowerSideMain, visible: e.target.checked }
                      }))}
                      className="mr-1"
                    />
                    Visible
                      ...
                      {['x', 'y', 'z'].map((axis) => (
                        <div key={axis} className="flex flex-col">
                          ...
                          <input
                            type="number"
                            step="0.1"
                            value={keyComponents.lowerSideMain.position[axis as keyof typeof keyComponents.lowerSideMain.position]}
                            onChange={(e) => setKeyComponents(prev => ({
                              ...prev,
                              lowerSideMain: {
                                ...prev.lowerSideMain,
                                position: { ...prev.lowerSideMain.position, [axis]: parseFloat(e.target.value) || 0 }
                              }
                            }))}
                            className="w-full px-1 py-0.5 text-xs bg-gray-700 border border-gray-600 rounded"
                          />
                      ...
                      {['x', 'y', 'z'].map((axis) => (
                        <div key={axis} className="flex flex-col">
                          ...
                          <input
                            type="number"
                            step="0.1"
                            value={keyComponents.lowerSideMain.rotation[axis as keyof typeof keyComponents.lowerSideMain.rotation]}
                            onChange={(e) => setKeyComponents(prev => ({
                              ...prev,
                              lowerSideMain: {
                                ...prev.lowerSideMain,
                                rotation: { ...prev.lowerSideMain.rotation, [axis]: parseFloat(e.target.value) || 0 }
                              }
                            }))}
                            className="w-full px-1 py-0.5 text-xs bg-gray-700 border border-gray-600 rounded"
                          />
              ...
              <div className="mb-3">
                <div className="flex items-center justify-between mb-1">
                  <h5 className="text-xs font-medium text-cyan-400">Handle Up Cover</h5>
                  <label className="flex items-center text-xs">
                    <input
                      type="checkbox"
                      checked={keyComponents.handleUpCover.visible}
                      onChange={(e) => setKeyComponents(prev => ({
                        ...prev,
                        handleUpCover: { ...prev.handleUpCover, visible: e.target.checked }
                      }))}
                      className="mr-1"
                    />
                    Visible
                      ...
                      {['x', 'y', 'z'].map((axis) => (
                        <div key={axis} className="flex flex-col">
                          ...
                          <input
                            type="number"
                            step="0.1"
                            value={keyComponents.handleUpCover.position[axis as keyof typeof keyComponents.handleUpCover.position]}
                            onChange={(e) => setKeyComponents(prev => ({
                              ...prev,
                              handleUpCover: {
                                ...prev.handleUpCover,
                                position: { ...prev.handleUpCover.position, [axis]: parseFloat(e.target.value) || 0 }
                              }
                            }))}
                            className="w-full px-1 py-0.5 text-xs bg-gray-700 border border-gray-600 rounded"
                          />
                      ...
                      {['x', 'y', 'z'].map((axis) => (
                        <div key={axis} className="flex flex-col">
                          ...
                          <input
                            type="number"
                            step="0.1"
                            value={keyComponents.handleUpCover.rotation[axis as keyof typeof keyComponents.handleUpCover.rotation]}
                            onChange={(e) => setKeyComponents(prev => ({
                              ...prev,
                              handleUpCover: {
                                ...prev.handleUpCover,
                                rotation: { ...prev.handleUpCover.rotation, [axis]: parseFloat(e.target.value) || 0 }
                              }
                            }))}
                            className="w-full px-1 py-0.5 text-xs bg-gray-700 border border-gray-600 rounded"
                          />
              ...
              <div className="mb-3">
                <div className="flex items-center justify-between mb-1">
                  <h5 className="text-xs font-medium text-cyan-400">Hair Guide Support</h5>
                  <label className="flex items-center text-xs">
                    <input
                      type="checkbox"
                      checked={keyComponents.hairGuideSupport.visible}
                      onChange={(e) => setKeyComponents(prev => ({
                        ...prev,
                        hairGuideSupport: { ...prev.hairGuideSupport, visible: e.target.checked }
                      }))}
                      className="mr-1"
                    />
                    Visible
                      ...
                      {['x', 'y', 'z'].map((axis) => (
                        <div key={axis} className="flex flex-col">
                          ...
                          <input
                            type="number"
                            step="0.1"
                            value={keyComponents.hairGuideSupport.position[axis as keyof typeof keyComponents.hairGuideSupport.position]}
                            onChange={(e) => setKeyComponents(prev => ({
                              ...prev,
                              hairGuideSupport: {
                                ...prev.hairGuideSupport,
                                position: { ...prev.hairGuideSupport.position, [axis]: parseFloat(e.target.value) || 0 }
                              }
                            }))}
                            className="w-full px-1 py-0.5 text-xs bg-gray-700 border border-gray-600 rounded"
                          />
                      ...
                      {['x', 'y', 'z'].map((axis) => (
                        <div key={axis} className="flex flex-col">
                          ...
                          <input
                            type="number"
                            step="0.1"
                            value={keyComponents.hairGuideSupport.rotation[axis as keyof typeof keyComponents.hairGuideSupport.rotation]}
                            onChange={(e) => setKeyComponents(prev => ({
                              ...prev,
                              hairGuideSupport: {
                                ...prev.hairGuideSupport,
                                rotation: { ...prev.hairGuideSupport.rotation, [axis]: parseFloat(e.target.value) || 0 }
                              }
                            }))}
                            className="w-full px-1 py-0.5 text-xs bg-gray-700 border border-gray-600 rounded"
                          />
